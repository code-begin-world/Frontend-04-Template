...
function test(source, pattern) {
    ...
    if (p === -1) {
        ...
        document.body.insertAdjacentHTML('beforeend', `<p><code>${source}</code>, <code>${pattern}</code> 不存在匹配</p><br/>`);
    } else {
        console.log(source, pattern, `存在匹配，开始位置为 ${p}`);
        var sourceHTML = '<code>' + source.substring(0, p) + `<span style="color:red">${source.substr(p, pattern.length)}</span>` + source.substr(p + pattern.length) + '</code>';
